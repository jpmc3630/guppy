...
export default {
  name: 'PageIndex',
  emits: ['dragstart, dragend, dragenter, touchmove'],
  data () {
    return {
      keyValue: 0,
      direction: 'next',
  ...
  mounted () {
    this.$eventBus.on('calendar:next', this.calendarNext)
  ...
  beforeUnmount () {
    this.$eventBus.off('calendar:next', this.calendarNext)
    this.$eventBus.off('calendar:prev', this.calendarPrev)
    this.$eventBus.off('calendar:today', this.calendarToday)
  },
  computed: {
    ...mapGetters({
